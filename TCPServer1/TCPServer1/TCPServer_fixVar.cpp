...
// 고정(4byte) + 가변 길이(배열에 있는 값) 방식

// 클라이언트 -> 서버 -> 클라이언트 -> 서버(1:1)
...
int main() {
	...
	char buf[BUFSIZE + 1];  // 전달할 데이터 저장공간
	int len;				// 고정 길이를 체크할 변수
	...
	while (1) {
		...
		while (1) {
			...
			//recvn(소켓, 읽을 공간, 크기 , 0) 데이터를 읽어들임
			retval = recvn(client_sock, (char *)&len, sizeof(int), 0); // 고정 길이 수신하기
			...
			retval = recvn(client_sock, buf , len, 0); // 가변 길이 수신하기
			if (retval == INVALID_SOCKET) {
				err_display("recvn() 오류");
				break;
			}
