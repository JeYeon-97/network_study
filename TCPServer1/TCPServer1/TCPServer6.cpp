...
#include <stdlib.h>
#include <stdio.h>
#include <WS2tcpip.h>  // WSAStringToAddress 함수 사용 
...
int main() {
	...
	SOCKADDR_IN6 serveraddr; // 서버구조체 변수
	ZeroMemory(&serveraddr, sizeof(serveraddr)); // 서버주소 변수를 0으로 초기화
	serveraddr.sin6_family = AF_INET6;  // 주소체계(IPv4, IPv6)
	serveraddr.sin6_addr = in6addr_any; // IP주소 설정 htonl : 호스트 -> 네트워크 바이트 정렬
									    // INADDR_ANY : 자동으로 ip주소 설정 (sin6_addr은 .s_addr 지원 x )
	serveraddr.sin6_port = htons(SERVERPORT); // 호스트 -> 네트워크 바이트로
	...
	SOCKADDR_IN6 clientaddr; // 연결된 클라이언트 주소
	int addrlen;			// 주소 길이
